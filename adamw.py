...
class AdamW(Optimizer):
    def __init__(
        ...
        *,
        ...
        param_to_index: Optional[dict] = None
    ):
        if not 0.0 <= lr:
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                state["exp_avg"] = torch.zeros(int(0.1 * p.numel()), device=p.device, dtype=p.dtype) if p.shape[0] != 151936 else torch.zeros_like(p, memory_format=torch.preserve_format)
                ...
                state["exp_avg_sq"] = torch.zeros(int(0.1 * p.numel()), device=p.device, dtype=p.dtype) if p.shape[0] != 151936 else torch.zeros_like(p, memory_format=torch.preserve_format)

                # state["update_index"] = torch.randperm(int(0.1 * p.numel()), device=p.device, dtype=torch.long)
            ...
            # update_indexs.append(state["update_index"])
...
def _multi_tensor_adamw(
    ...
    *,
    ...
    for ((
        ...
        # Decay the first and second moment running average coefficient
        # TODO：这里是一阶动量的更新，需要修改
        # torch._foreach_lerp_(device_exp_avgs, device_grads, 1 - beta1)

        # for i, (param,exp_avg, grad) in enumerate(zip(device_params, device_exp_avgs, device_grads)):
        #     # 实现上面的代码，注意原地操作
        #     update_index = optimizer.state[param]['update_index']
        #     exp_avg.lerp_(torch.take(grad, update_index), 1 - beta1)

        # TODO：这里是二阶动量的更新，需要修改
        torch._foreach_mul_(device_exp_avg_sqs, beta2) # 这个不需要修改
        # torch._foreach_addcmul_(device_exp_avg_sqs, device_grads, device_grads, 1 - beta2)

        # 将一阶和二阶动量更新合并到一起
        for i, (param,exp_avg, exp_avg_sq, grad) in enumerate(zip(device_params, device_exp_avgs, device_exp_avg_sqs, device_grads)):
            # 实现上面的代码，注意原地操作
            if param.shape[0] == 151936:
                # 对它执行全局更新
                exp_avg.lerp_(grad, 1 - beta1)
                ...
                if weight_decay != 0:
                    param.mul_(1 - lr * weight_decay)
                continue
            update_index = optimizer.param_to_index[param]
            temp_grad = torch.take(grad, update_index)
            exp_avg.lerp_(temp_grad, 1 - beta1)
            exp_avg_sq.addcmul_(temp_grad, temp_grad, value=1 - beta2)
            

            # 权重衰减,只对update_index指定的参数进行权重衰减
            if weight_decay != 0:
                param.view(-1).scatter_(0, update_index, torch.take(param, update_index) * (1 - lr * weight_decay))
        ...
        if capturable:
            ...
            torch._foreach_div_(exp_avg_sq_sqrt, bias_correction2_sqrt)
            torch._foreach_add_(exp_avg_sq_sqrt, eps)
            # TODO: 这里是更新参数的地方，需要修改。因为一阶动量和二阶动量不再和参数形状一样
            # torch._foreach_addcdiv_(device_params, device_exp_avgs, exp_avg_sq_sqrt, step_size)
            for param, exp_avg, exp_avg_sq, s in zip(device_params, device_exp_avgs, exp_avg_sq_sqrt, step_size):
                if param.shape[0] == 151936:
                    # 对它执行全局更新
                    param.addcdiv_(exp_avg, exp_avg_sq, value=s)
                    continue
                update_index = optimizer.param_to_index[param]
                param.view(-1).scatter_add_(0, update_index, exp_avg * s / exp_avg_sq)
