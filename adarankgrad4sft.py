...
from adagradrank_adamw import AdaptiveAdamW
...
# 加载模型和分词器
model_path = f"/HOME/hitsz_qcchen/hitsz_qcchen_1/new_disk/SPUpdater-main/fined_chkpts/adarankgrad/{task_name[task_id-1]}"
...
for module_name, module in model.named_modules():
    ...
    print('enable AdaRankGrad for weights in module: ', module_name)
    adaptive_params.append(module.weight)
id_adaptive_params = [id(p) for p in adaptive_params]
# make parameters without "rank" to another group
regular_params = [p for p in model.parameters() if id(p) not in id_adaptive_params]
...
optimizer = AdaptiveAdamW(param_groups, lr=1e-5, weight_decay=0.0, grad_matrix_norm_thresh=0.0001)

# 训练参数配置
trainer_args = TrainingArguments(
    output_dir=f"/HOME/hitsz_qcchen/hitsz_qcchen_1/new_disk/SPUpdater-main/fined_chkpts/adarankgrad/{task_name[task_id]}",
    num_train_epochs=1,
...
# 保存模型
model.save_pretrained(f"/HOME/hitsz_qcchen/hitsz_qcchen_1/new_disk/SPUpdater-main/fined_chkpts/adarankgrad/{task_name[task_id]}")
