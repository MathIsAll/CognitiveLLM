...
lora_config = LoraConfig(
    use_dora=True,  # to use Dora OR compare to Lora just set the --use_dora
    r=16,  # Rank of matrix
    lora_alpha=32,
...
"""
peft_model_path = f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/dora_peft/{task_name[task_id-1]}"
model = PeftModel.from_pretrained(model, peft_model_path, is_trainable=True)

trainer_args = TrainingArguments(
    output_dir=f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/dora_peft/{task_name[task_id]}",
    num_train_epochs=1,
...
model.save_pretrained(f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/dora_peft/{task_name[task_id]}")
