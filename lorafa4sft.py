...
"""
peft_model_path = f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/lorafa_peft/{task_name[task_id-1]}"
model = PeftModel.from_pretrained(model, peft_model_path, is_trainable=True)

# 训练参数配置
trainer_args = TrainingArguments(
    output_dir=f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/lorafa_peft/{task_name[task_id]}",
    num_train_epochs=1,
...
optimizer = create_lorafa_optimizer(model=model, r=16, lora_alpha=32, lr=1e-5, weight_decay=0.01)
...
# 保存模型
model.save_pretrained(f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/lorafa_peft/{task_name[task_id]}")
