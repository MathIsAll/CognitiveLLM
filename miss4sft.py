...
from peft import MissConfig, get_peft_model, PeftModel
import torch
import torch.optim
...
# 指定包含所有JSON文件的目录路径
"""
folder_path = "/HOME/hitsz_qcchen/hitsz_qcchen_1/new_disk/SPUpdater-main/ds_distilled_samples/python-code-dataset-500k.json"
...
# 应用LoRA到模型[3,4](@ref)
model = get_peft_model(model, miss_config)
"""
peft_model_path = f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/miss_peft/{task_name[task_id-1]}"
model = PeftModel.from_pretrained(model, peft_model_path, is_trainable=True)

trainer_args = TrainingArguments(
    output_dir=f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/miss_peft/{task_name[task_id]}",
    num_train_epochs=1,
...
model.save_pretrained(f"/HOME/hitsz_qcchen/hitsz_qcchen_1/HDD_POOL/SPUpdater-main/fined_chkpts/peft_chkpts/miss_peft/{task_name[task_id]}")
